...
int main(int argc,char *argv[])
{
	...
	else if(pid>0)
	{
		...
		read(c2p[0],&rev,sizeof(rev));							// recieve reversed string from parent
		printf("recieved reverse from child in parent\n");
		printf("original string : '%s'\n",string);
		printf("reversed string : '%s'\n",rev);
		printf("string is%s a palindrome\n",strcmp(string,rev)?" not":"");	// print result
	}
	else
	{
		char string[size],rev[size];
